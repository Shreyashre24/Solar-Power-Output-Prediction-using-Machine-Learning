# -*- coding: utf-8 -*-
"""code solar power output prediction.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11gxU6R-8vKsQKhb2A2-6C8DA33tg0R7Y
"""
...
def evaluate(y_true, y_pred, eps=1e-6):
    ...
    rmse = np.sqrt(mse)   # FIX: No squared= argument
...
def run_pipeline(df, seq_lags=[1,2], test_days=30, random_state=42):
    ...
    # MLP Neural Net
    mlp = MLPRegressor(hidden_layer_sizes=(64,32), max_iter=200, random_state=random_state)
    mlp.fit(X_train, y_train)
    results['MLP'] = evaluate(y_test, mlp.predict(X_test))


    # ARIMA (just on training series)
    try:
        arima_model = ARIMA(train['power'], order=(3,1,2)).fit()
        y_pred_arima = arima_model.forecast(steps=len(test))
        results['ARIMA'] = evaluate(y_test, y_pred_arima)
...
if __name__ == "__main__":
    ...
    print("Running pipeline on 10-day dataset")
